...
Spring AOP 就是基于动态代理的如果要代理的对象，实现了某个接口，那么 Spring AOP 会使用 **JDK Proxy**，去创建代理对象，而对于没有实现接口的对象，就无法使用 JDK Proxy 去进行代理了，这时候 Spring AOP 会使用 **Cglib** 生成一个被代理对象的子类来作为代理，如下图所示
...
AOP 切面编程涉及到的一些专业术语：**通知相当于增强**

1. 目标(Target)   被通知的对象代理
2. (Proxy)向目标对象应用通知之后创建的代理对象连接点
3. (JoinPoint)目标对象的所属类中，定义的所有方法均为连接点切入点
4. (Pointcut)被切面拦截 / 增强的连接点（切入点一定是连接点，连接点不一定是切入点）
5. 通知(Advice)增强的逻辑 / 代码，也即拦截到目标对象的连接点之后要做的事情切面
6. (Aspect)切入点(Pointcut)+通知(Advice)
...
不使用IOC的话就要，service层要使用dao层的话就要new 对象，如果后面方法更改的话就要更新。

而使用IOC的话就可以通过接口就可以实现功能，

使用 IoC 的思想，我们将对象的控制权（创建、管理）交有 IoC 容器去管理，我们在使用的时候直接向 IoC 容器 “要” 就可以了

![img](https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/ioc-kfji3.png)

![img](https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/why-ioc.png)

![img](https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/why-ioc-2.png)

IoC 最常见以及最合理的实现方式叫做依赖注入（Dependency Injection，简称 DI

### AOP

AOP（Aspect Oriented Programming）即面向切面编程，是oop的一种延续
